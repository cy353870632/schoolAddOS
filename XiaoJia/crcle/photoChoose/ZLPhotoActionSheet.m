...
//    [self showPreview:YES sender:sender animate:animate lastSelectPhotoModels:self.arratYijingChoosePhoto completion:completion];
}
...
- (void)showPreview:(BOOL)preview sender:(UIViewController *)sender animate:(BOOL)animate lastSelectPhotoModels:(NSArray<ZLSelectPhotoModel *> *)lastSelectPhotoModels completion:(void (^)(NSArray<UIImage *> * _Nonnull, NSArray<ZLSelectPhotoModel *> * _Nonnull))completion
{
    ...
    if (self.arratYijingChoosePhoto.count == 0) {
        lastSelectPhotoModels = self.arratYijingChoosePhoto;
    }else{
//    [self.arraySelectPhotos addObjectsFromArray:self.arratYijingChoosePhoto];
    }
