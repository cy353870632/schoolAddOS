...
@interface CrcleViewController ()<GrayBackgroundViewDeleget,UIImagePickerControllerDelegate,UIActionSheetDelegate,UINavigationControllerDelegate,UITableViewDelegate,UITableViewDataSource,CellForWorkGroupDelegate,CellForWorkGroupRepostDelegate>
{
    //朋友圈页面
...
@implementation CrcleViewController
...
- (void)viewDidLoad {
    ...
    UIBarButtonItem * addCrcleButtonItem = [[UIBarButtonItem alloc]initWithCustomView:self.addCrcle_btn];
    self.navigationItem.rightBarButtonItem = addCrcleButtonItem;
...
//懒加载
-(UIButton *)addCrcle_btn
{
    if (!_addCrcle_btn) {
        _addCrcle_btn = [UIButton buttonWithType:UIButtonTypeContactAdd];
        _addCrcle_btn.tintColor = [UIColor whiteColor];
        [_addCrcle_btn addTarget:self action:@selector(addNewCrcle) forControlEvents:UIControlEventTouchUpInside];
    }
    return _addCrcle_btn;
}
//添加朋友圈页面
-(void)addCrcle{
    [self initUI];
...
-(void)addNewCrcle{
    NSLog(@"新建朋友圈");
    PublishCrcleViewController * publishCrcleViewController = [[PublishCrcleViewController alloc]init];
    [self.navigationController pushViewController:publishCrcleViewController animated:YES];
}
...
- (void)initUI{
    ...
    self.tableView = [[YHRefreshTableView alloc] initWithFrame:CGRectMake(0, 0, MAINSCREEN.width, MAINSCREEN.height) style:UITableViewStylePlain];
