...
- (void)setupData {
    ...
    [self.tableViewData addObject:@[model,model1]];
}
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (buttonIndex==0) {
        ...
        NSLog(@"创建群组");
        AddGroupViewController * addGroupViewController = [[AddGroupViewController alloc]init];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (buttonIndex==1) {
        ...
        if (isSuccess && !error) {
            [self.view makeToast:[NSString stringWithFormat:@"发送成功"] duration:1.5 position:@"center"];//在中间显示土司
            NSLog(@"添加(%@)好友成功", textfield.text);
        }else {
            [self.view makeToast:@"添加好友失败,请稍后重试" duration:1.5 position:@"center"];//在中间显示土司
            NSLog(@"添加好友失败");
        }
    }
}
...
- (UIButton *)navRightButton {
    if (!_navRightButton) {
        ...
        _navRightButton.layer.cornerRadius = _navRightButton.currentBackgroundImage.size.width/2;
    }
