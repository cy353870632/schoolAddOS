...
//    //返回按钮
//    UIImage * backimage = [UIImage imageNamed:@"fhan_01.png"];
//    backimage = [backimage imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
//    UIBarButtonItem * backButton = [[UIBarButtonItem alloc]initWithImage:backimage style:UIBarButtonItemStyleDone target:self action:@selector(backbutton_click)];
//    self.navigationItem.leftBarButtonItem = backButton;
    ...
    if (self.conversation.conversationType == eConversationTypeGroupChat || self.conversation.conversationType == eConversationTypeChatRoom) {
        NSLog(@"%@",self.conversation.chatter);
        UIImage * roomMessage_img = [UIImage imageNamed:@"icon_contact_normal"];
        
        UIButton * roomMessagebutton = [UIButton buttonWithType:UIButtonTypeSystem];
        roomMessagebutton.frame = CGRectMake(0, 0, 50, 50);
        [roomMessagebutton setImage:roomMessage_img forState:UIControlStateNormal];
        roomMessagebutton.tintColor = [UIColor whiteColor];
        [roomMessagebutton addTarget:self action:@selector(getCharRoomMessage) forControlEvents:UIControlEventTouchUpInside];
        
        UIBarButtonItem *backBarButtonItem = [[UIBarButtonItem alloc]initWithCustomView:roomMessagebutton];
...
//获取群组详情
-(void)getCharRoomMessage{
    NSLog(@"获取该群组信息");
    EMError *error = nil;
    EMGroup * group = [[EaseMob sharedInstance].chatManager fetchGroupInfo:self.conversation.chatter error:&error];
    if (!error) {
        NSLog(@"群组名称：%@",group.groupSubject);
        NSLog(@"群组id：%@",group.groupId);
        NSLog(@"群组描述：%@",group.groupDescription);
        NSLog(@"群组总人数：%ld",(long)group.groupOccupantsCount);
    }else
    {
        NSLog(@"获取群信息失败");
    }
