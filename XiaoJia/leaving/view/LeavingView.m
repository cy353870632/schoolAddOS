...
@interface LeavingView()<UITableViewDelegate,UITableViewDataSource>
{
    LeavingTableViewCell * cell;
}
@end
@implementation LeavingView
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            [twobutton[i] setFrame:CGRectMake(MAINSCREEN.width/2*i,MAINSCREEN.height/10, MAINSCREEN.width/2,40)];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!cell) {
        cell = [[LeavingTableViewCell alloc]initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:cellIdentifier];
    }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self.deleget didSelectCell:indexPath.section];
}
