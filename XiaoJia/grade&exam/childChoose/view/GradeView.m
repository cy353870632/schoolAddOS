...
@interface GradeView ()<WYPieChartViewDelegate,WYPieChartViewDatasource,UITableViewDelegate,UITableViewDataSource>
{
    Marquee * paomav;
}
...
@property(nonatomic,strong)PeiXunViewForGradeView * peiXunViewForGradeView;
@end
...
-(void)initShanXingTU
{
    ...
    _pieView = [[WYPieChartView alloc] initWithFrame:CGRectMake(40, CGRectGetMaxY(paomav.frame)-20, MAINSCREEN.width-80, MAINSCREEN.width-80)];
...
- (void)pieChartView:(WYPieChartView *)pieChartView didSelectedSectorAtIndex:(NSInteger)index{
    NSLog(@"%ld",(long)index);
    if ([_labelBackgroundColor[index] isEqual:green_color]) {
    ...
    {
         [self makeToast:@"该科暂未点评" duration:1.5 position:@"center"];//在中间显示土司
    }
...
#pragma mark 懒加载
-(UIButton *)gradeBaoGao_button
{
    if (!_gradeBaoGao_button) {
        UIImage * gradeBaoGao_img = [UIImage imageNamed:@"gradebaogao"];
        //原图展示
        gradeBaoGao_img = [gradeBaoGao_img imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
        
        _gradeBaoGao_button = [UIButton buttonWithType:UIButtonTypeCustom];
        _gradeBaoGao_button.frame = CGRectMake(20, CGRectGetMaxY(_pieView.frame)-40, 50, 50);
        _gradeBaoGao_button.layer.cornerRadius = 30;
        [_gradeBaoGao_button setImage:gradeBaoGao_img forState:UIControlStateNormal];
        [_gradeBaoGao_button addTarget:self action:@selector(gradeBaoGaoClick) forControlEvents:UIControlEventTouchUpInside];
    }
    return _gradeBaoGao_button;
}
-(UIButton *)liShiShuJu_button
{
    if (!_liShiShuJu_button) {
        UIImage * gradeBaoGao_img = [UIImage imageNamed:@"lishichengji"];
        //原图展示
        gradeBaoGao_img = [gradeBaoGao_img imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
        
        _liShiShuJu_button = [UIButton buttonWithType:UIButtonTypeCustom];
        _liShiShuJu_button.frame = CGRectMake(MAINSCREEN.width-70, CGRectGetMaxY(_pieView.frame)-40, 50, 50);
        _liShiShuJu_button.layer.cornerRadius = 30;
        [_liShiShuJu_button setImage:gradeBaoGao_img forState:UIControlStateNormal];
        [_liShiShuJu_button addTarget:self action:@selector(lishiShuJuClick) forControlEvents:UIControlEventTouchUpInside];
    }
    return _liShiShuJu_button;
}
...
-(UITableView *)GradeList_tableView
{
    if (!_GradeList_tableView) {
        _GradeList_tableView = [[UITableView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(self.liShiShuJu_button.frame)+4, MAINSCREEN.width, 40*_subject_array.count+6) style:UITableViewStylePlain];
...
{
    if (!_notice_label) {
        _notice_label = [[UILabel alloc]initWithFrame:CGRectMake(10,CGRectGetMaxY(_GradeList_tableView.frame), MAINSCREEN.width-20, 40)];
        ...
        _notice_label.text = @"     系统根据本次成绩，为您挑选推荐如下成绩提高方案，请您根据自身需要选择相应方案";
        [_notice_label setFont:[UIFont fontWithName:@"Helvetica-Bold" size:14]];
    }
    return _notice_label;
}

-(PeiXunViewForGradeView *)peiXunViewForGradeView
{
    if (!_peiXunViewForGradeView) {
        NSLog(@"%f",self.frame.size.height);
        NSLog(@"%f",CGRectGetMaxY(_notice_label.frame));
        _peiXunViewForGradeView = [[PeiXunViewForGradeView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(_notice_label.frame)+3, MAINSCREEN.width, MAINSCREEN.height*2-CGRectGetMaxY(_notice_label.frame)-30)];
        ...
        _peiXunViewForGradeView.shopping_array = self.shopping_array;
        
    }
    return _peiXunViewForGradeView;
}
...
#pragma mark 点击事件
-(void)gradeBaoGaoClick
{
    NSLog(@"成绩报告");
    [self.selectWYCdelegate gradeBaoGao];
}
-(void)lishiShuJuClick
{
    NSLog(@"历史成绩");
    [self.selectWYCdelegate historyGrade];
}
@end
